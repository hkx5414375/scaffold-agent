...
Scaffold Agent is under active construction. The protocol core, deterministic filesystem transactions, stable JSON CLI, six-tool MCP server, and the Go/PostgreSQL/MySQL generator are implemented. Generated applications include secure identity, permission RBAC, transactional audit, Blueprint-driven CRUD, OpenAPI 3.1, and a build-tested Vue/Element Plus administration UI. Versioned capabilities now provide tenant isolation, secure organization lifecycle, durable background workers, and idempotent TLS-only email notifications. Nuxt storefronts and broader capability packs remain in progress; schemas stay experimental until 1.0.
