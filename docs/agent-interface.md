...
Selecting `crm-core` version `0.1.0` with the Go or Java adapter generates business
...
immutable, and never skip pipeline stages. Python rejects this capability until
its parity implementation is complete. See `examples/crm-service-go/scaffold.yaml`
and `examples/crm-service-java/scaffold.yaml` for complete selections.
