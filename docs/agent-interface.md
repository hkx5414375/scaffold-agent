...
`examples/task-service-python/scaffold.yaml` for the equivalent Java and Python
contracts.
...
The Go, Java, and Python `0.2.0` versions additionally generate organization member discovery, 72-hour
...
(`notifications.Service.EnqueueEmail` in Go, `NotificationService.enqueueEmail`
in Java, or `NotificationService.enqueue_email` in Python) with a stable idempotency key; it must not expose a generic unauthorised email
...
generated file service (`files.Service` in Go, or `FileAssetService` in Java and Python); it
...
`CacheService` in Java and Python, should namespace keys, select a stable TTL, and treat a
...
Selecting `csv-import-export` version `0.1.0` on Go, Java, or Python requires one generated business
...
Selecting `approval-workflows` version `0.1.0` on Go, Java, or Python requires one generated business
...
Selecting `storefront: nuxt` generates the same `web/storefront` Nuxt 4 SSR
foundation for Go, Java, and Python and records `nuxt-storefront` `0.1.0` in the
capability lock. Its backend base URL is server-only `SCAFFOLD_API_BASE_URL`; the
browser calls a bounded local status endpoint, and upstream locations or failures
are never exposed. The generated project pins its npm graph and must pass ESLint,
Vitest, Nuxt type checking, production build, Prettier, and dependency audit.
Agents should extend this owned foundation through later storefront capability
...
Selecting `crm-core` version `0.1.0` with the Go, Java, or Python adapter generates business
...
immutable, and never skip pipeline stages. A cross-backend conformance gate keeps
the OpenAPI operations, permissions, core schemas, migration tables, and shared
administration view aligned. See `examples/crm-service-go/scaffold.yaml`,
`examples/crm-service-java/scaffold.yaml`, and
`examples/crm-service-python/scaffold.yaml` for complete selections.

Selecting `erp-inventory` version `0.1.0` with the Go, Java, or Python adapter generates inventory
...
`examples/inventory-service-go-mysql/scaffold.yaml`,
`examples/inventory-service-java/scaffold.yaml`,
`examples/inventory-service-python/scaffold.yaml`, and
`examples/inventory-service-python-mysql/scaffold.yaml` for complete reference selections.
