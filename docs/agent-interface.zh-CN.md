...
Go 适配器选择 `erp-inventory` `0.1.0` 后，会为 PostgreSQL 或 MySQL 生成库存品、仓库、整数库存余额、不可变库存流水、库存预留、采购单、OpenAPI 和共用 Vue 管理端。Blueprint 应在 `project.capabilities` 中精确锁定该版本；租户项目还必须选择 `organization-tenancy` `0.3.0`。数量和乐观版本均为 `int64`，在 JSON 中使用十进制字符串，AI 不得引入浮点库存。调整、预留、释放、消耗和采购收货都必须携带幂等键：同一命令重放会返回首次提交的结果，换参数复用同一键则返回冲突。一次数据库事务必须同时维持 `on_hand >= 0`、`reserved >= 0`、`reserved <= on_hand`、`available = on_hand - reserved`，并写入库存流水与审计。AI 应直接复用 `inventory:read`、`inventory:catalog:write`、`inventory:stock:manage`、`inventory:procurement:manage` 四个权限，保持流水只追加，也不得擅自把库存品和商城商品绑定。当前 Go 参考选择见 `examples/inventory-service-go/scaffold.yaml` 与 `examples/inventory-service-go-mysql/scaffold.yaml`；Java、Python 对齐进度以路线图为准。
