...
六个工具和存储协议已实现。第一个 Go 适配器已能生成带 PostgreSQL、内嵌迁移、Session 与 Token 双认证、权限码 RBAC 及审计事件的 HTTP 服务。对尚未支持的数据库、前端、认证方式和业务模块，它会明确拒绝，不会静默漏生代码。Java 和 Python 目前会返回稳定诊断 `generator.adapter.unavailable`。
